...
const devicesId = "642995351" // 填写在OneNet上获得的devicesId 形式就是一串数字 例子:9939133
const api_key = "GR6=PYb0DPL8Zt2i6dYJm0PCR78=" // 填写在OneNet上的 api-key 例子: VeFI0HZ44Qn5dZO14AuLbWSlSlI=
...
Page({
      ...
      wx.request({
        ...
        success: (res) => {
          ...
          resolve({
            ...
            distance: response.data.datastreams[1].datapoints.reverse()
          })
        },
        fail: (err) => {
          reject(err)
  ...
  convert: function (datapoints) {
    ...
    for (var i = 0; i < length; i++) {
      categories.push(datapoints.distance[i].at.slice(5, 19));
